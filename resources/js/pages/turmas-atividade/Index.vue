...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Turmas Atividades', href: '/turmas-atividade' }];
...
const apply = (resetPage = false) => {
    ...
    timer = setTimeout(() => {
        ...
        router.get('/turmas-atividade', params, { preserveState: true, replace: true });
    }, 100);
};
...
const remove = (t: Turma) => {
    if (!confirm(`Remover turma de atividade "${t.tur_nome}"?`)) return;
    router.delete(`/turmas-atividade/${t.tur_id}`, { preserveScroll: true });
};
...
<template>
    <Head title="Turmas Atividades" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 md:p-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">Turmas Atividades</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Atividades Complementares · vinculadas à matrícula regular.</p>
                </div>
                <Link href="/turmas-atividade/create">
                    <Button class="bg-teal-600 text-white hover:bg-teal-700">
                        <Plus class="mr-2 size-4" /> Nova Turma Atividade
                    </Button>
                </Link>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="ml-auto flex items-center gap-3">
                    ...
                    <ExportMenu base-url="/turmas-atividade/export" :filters="exportFilters()" />
                </div>
            </div>
            ...
            <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-if="turmas.data.length === 0">
                            <td colspan="8" class="px-4 py-12 text-center text-muted-foreground">Nenhuma turma de atividade encontrada.</td>
                        </tr>
                        <tr v-for="t in turmas.data" :key="t.tur_id" class="transition-colors hover:bg-muted/30">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="grid size-9 place-items-center rounded-lg bg-teal-100 text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">
                                        <Activity class="size-4" />
                                    </div>
                                    <span class="font-semibold">{{ t.tur_nome }}</span>
                                </div>
                            </td>
                            <td class="max-w-[260px] truncate px-4 py-3 text-muted-foreground">{{ t.escola?.esc_nome ?? '—' }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <Link :href="`/turmas-atividade/${t.tur_id}/edit`">
                ...
                <div class="flex items-center justify-between border-t bg-muted/20 px-4 py-3 text-sm">
                    ...
                    <div v-if="turmas.last_page > 1" class="flex flex-wrap gap-1">
                        <Link v-for="(link, i) in turmas.links" :key="i" :href="link.url ?? '#'" v-html="link.label" :class="['rounded-md px-3 py-1 text-xs', link.active ? 'bg-teal-600 text-white' : 'border bg-background hover:bg-muted', !link.url && 'pointer-events-none opacity-40']" preserve-scroll />
                    </div>
                </div>
            </div>
        </div>
