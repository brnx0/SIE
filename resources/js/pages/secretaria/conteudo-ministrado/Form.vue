...
async function loadTurmas() {
    ...
    turmas.value = await getJson(`/secretaria/conteudo-ministrado/turmas?esc_id=${escId.value}&anl_id=${anlId.value}`);
}
async function loadUnidades() {
    unidades.value = []; uniId.value = null;
    if (!anlId.value) return;
    unidades.value = await getJson(`/secretaria/conteudo-ministrado/unidades?anl_id=${anlId.value}`);
}
...
function gerar() {
    ...
    router.get('/secretaria/conteudo-ministrado/gerar', payload, { preserveScroll: true, onFinish: () => { gerando.value = false; } });
}
</script>

<template>
    <Head title="Conteúdo Ministrado" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-[95%] py-6">
            <h1 class="mb-1 flex items-center gap-2 text-xl font-semibold">
                <BookOpenText class="size-5 text-indigo-600" /> Conteúdo Ministrado
            </h1>
            <p class="mb-6 text-sm text-muted-foreground">Por disciplina, lista os dias com aula registrada na frequência, o planejamento executado e o conteúdo/metodologia do dia — para qualquer turma da escola. Período em branco = todos.</p>
