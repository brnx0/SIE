...
async function loadTurmas() {
    ...
    turmas.value = await getJson(`/relatorios/conteudo-ministrado/turmas?esc_id=${escId.value}&anl_id=${anlId.value}`);
}
async function loadUnidades() {
    unidades.value = []; uniId.value = null;
    if (!anlId.value) return;
    unidades.value = await getJson(`/relatorios/conteudo-ministrado/unidades?anl_id=${anlId.value}`);
}
...
function gerar() {
    ...
    router.get('/relatorios/conteudo-ministrado/gerar', payload, { preserveScroll: true, onFinish: () => { gerando.value = false; } });
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-[95%] py-6">
            ...
            <p class="mb-6 text-sm text-muted-foreground">Por disciplina da turma, lista os dias com aula registrada na frequência, o planejamento executado e o conteúdo/metodologia do dia. Período em branco = todos.</p>
