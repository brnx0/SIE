...
async function loadTurmas() {
    turmas.value = []; turId.value = null; alnId.value = null; alunos.value = [];
    if (!escId.value || !anlId.value) return;
    turmas.value = await getJson(`/relatorios/boletim/turmas?esc_id=${escId.value}&anl_id=${anlId.value}`);
}
async function loadUnidades() {
    unidades.value = []; uniId.value = null;
    if (!anlId.value) return;
    unidades.value = await getJson(`/relatorios/boletim/unidades?anl_id=${anlId.value}`);
}
async function loadAlunos() {
    alnId.value = null; alunos.value = [];
    if (!turId.value) return;
    alunos.value = await getJson(`/relatorios/boletim/alunos?tur_id=${turId.value}`);
}
...
function gerar() {
    ...
    router.get('/relatorios/boletim/gerar', payload, { preserveScroll: true, onFinish: () => { gerando.value = false; } });
}
</script>

<template>
    <Head title="Boletim do Aluno" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-[95%] py-6">
            <h1 class="mb-1 flex items-center gap-2 text-xl font-semibold">
                <GraduationCap class="size-5 text-indigo-600" /> Boletim do Aluno
            </h1>
            <p class="mb-6 text-sm text-muted-foreground">Notas e conceitos por disciplina e período. Deixe o período em branco para o consolidado do ano.</p>
