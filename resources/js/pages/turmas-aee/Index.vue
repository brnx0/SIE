...
import { Accessibility, Pencil, Plus, Trash2 } from 'lucide-vue-next';
...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Turmas AEE', href: '/turmas-aee' }];
...
const apply = (resetPage = false) => {
    ...
    timer = setTimeout(() => {
        ...
        router.get('/turmas-aee', params, { preserveState: true, replace: true });
    }, 100);
};
...
const remove = (t: Turma) => {
    if (!confirm(`Remover turma AEE "${t.tur_nome}"?`)) return;
    router.delete(`/turmas-aee/${t.tur_id}`, { preserveScroll: true });
};
...
<template>
    <Head title="Turmas AEE" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 md:p-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">Turmas AEE</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Atendimento Educacional Especializado.</p>
                </div>
                <Link href="/turmas-aee/create">
                    <Button class="bg-indigo-600 text-white hover:bg-indigo-700">
                        <Plus class="mr-2 size-4" /> Nova Turma AEE
                    </Button>
                </Link>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="ml-auto flex items-center gap-3">
                    ...
                    <ExportMenu base-url="/turmas-aee/export" :filters="exportFilters()" />
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-if="turmas.data.length === 0">
                            <td colspan="8" class="px-4 py-12 text-center text-muted-foreground">Nenhuma turma AEE encontrada.</td>
                        </tr>
                        <tr v-for="t in turmas.data" :key="t.tur_id" class="transition-colors hover:bg-muted/30">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <Link :href="`/turmas-aee/${t.tur_id}/edit`">
