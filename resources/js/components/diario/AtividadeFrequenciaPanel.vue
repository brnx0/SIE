...
const carregar = async (silent = false) => {
    ...
    erro.value = null;
    try {
        const url = new URL('/api/diario-atividade/frequencia/contexto', window.location.origin);
...
const marcar = async (aln: number, val: boolean) => {
    ...
    status[key] = 'saving';
    try {
        const r = await fetch('/api/diario-atividade/frequencia/salvar', {
            method: 'POST',
...
const marcarLote = async (val: boolean) => {
    ...
    try {
        const r = await fetch('/api/diario-atividade/frequencia/lote', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Accept: 'application/json', ...csrf() },
            credentials: 'same-origin',
...
<template>
    <section class="rounded-xl border bg-card shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b bg-card/95 px-4 py-3">
            <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-50">Frequência da Atividade</h2>
        ...
        <div class="p-4">
            ...
            <div v-else-if="!dias.length" class="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
                ...
                <span>Esta turma de atividade não tem dias de atendimento definidos.</span>
            </div>
            <div v-else-if="!alunos.length" class="py-12 text-center text-sm text-muted-foreground">Nenhum aluno na turma de atividade.</div>
