...
const carregar = async () => {
    ...
    erro.value = null;
    try {
        const r = await fetch(`/api/coordenador-interno/planos-aee/${props.planoId}`, { headers: { Accept: 'application/json' } });
        if (!r.ok) {
        ...
        statusOriginal.value = json.plano.dae_status;
        statusNovo.value = json.plano.dae_status;
        obs.value = json.plano.dae_obs_coordenador ?? '';
    } finally {
...
const salvar = () => {
    ...
    enviando.value = true;
    erros.value = {};
    router.put(`/coordenador-interno/planos-aee/${props.planoId}/status`, {
        dae_status: statusNovo.value,
        dae_obs_coordenador: obs.value,
    }, {
        preserveScroll: true,
        preserveState: true,
        ...
        <div class="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-card shadow-2xl">
            ...
            <div class="flex items-center justify-between border-b bg-muted/40 px-5 py-3">
                <div class="flex items-center gap-3">
                    <h2 class="text-lg font-semibold">Revisão de Plano AEE</h2>
                    <span v-if="plano" :class="['inline-flex rounded-full px-2.5 py-1 text-xs font-semibold', statusBadge(plano.dae_status)]">
                        {{ statusLabel(plano.dae_status) }}
                    </span>
                </div>
                ...
                <div v-else-if="plano" class="flex flex-col gap-5">
                    ...
                    <section class="rounded-lg border bg-background p-4">
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-3 text-sm">
                            ...
                            <div>
                                <div class="text-xs text-muted-foreground">Turma AEE</div>
                                <div>{{ plano.turma?.tur_nome ?? '—' }}</div>
                            </div>
                            <div class="md:col-span-2">
                                <div class="text-xs text-muted-foreground">Tema</div>
                                <div class="font-semibold">{{ plano.dae_tema }}</div>
                            </div>
                            <div>
                                <div class="text-xs text-muted-foreground">Período</div>
                                <div class="tabular-nums">{{ fmtDate(plano.dae_dt_inicio) }} – {{ fmtDate(plano.dae_dt_fim) }}</div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="grid grid-cols-1 gap-3 md:grid-cols-2">
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Objetivo das Intervenções</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_objetivo || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Diagnóstico da Deficiência</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_diagnostico || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Área de Desenvolvimento</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_area_desenv || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Metas de Aprendizagem</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_metas || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Estratégias de Intervenção</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_estrategias || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Recursos Utilizados</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_recursos || '—' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background p-4 md:col-span-2">
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Avaliação Feita Durante a Intervenção</h3>
                            <p class="whitespace-pre-wrap text-sm">{{ plano.dae_avaliacao || '—' }}</p>
                        </div>
                    </section>

                    <!-- Bloco Revisão -->
                    <section class="rounded-lg border-2 border-fuchsia-200 bg-fuchsia-50/40 p-4 dark:border-fuchsia-900 dark:bg-fuchsia-950/30">
                        <h3 class="mb-3 text-sm font-semibold text-fuchsia-900 dark:text-fuchsia-200">Revisão</h3>
                        ...
                        <div v-else-if="!podeRevisar" class="rounded-md bg-amber-100 p-3 text-sm text-amber-900">
                            ...
                            <div v-if="plano?.validado_por || plano?.dae_validado_em" class="mt-2 text-xs">
                                Validado por <span class="font-semibold">{{ plano?.validado_por?.name ?? '—' }}</span>
                                em <span class="font-semibold">{{ fmtDateTime(plano?.dae_validado_em) }}</span>
                            </div>
                        </div>
                        <div v-else class="grid grid-cols-1 gap-3 md:grid-cols-3">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <InputError :message="erros.dae_obs_coordenador" />
                            </div>
                        </div>
                    </section>
                </div>
            ...
            <div class="flex items-center justify-between border-t bg-muted/40 px-5 py-3">
                ...
                <Button :disabled="!podeSalvar"
                    class="bg-fuchsia-600 text-white hover:bg-fuchsia-700 disabled:opacity-50"
                    @click="salvar">
