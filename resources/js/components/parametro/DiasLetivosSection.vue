...
import type { AnoLetivo, DiasLetivos, Unidade } from '@/types/parametro';
import { router } from '@inertiajs/vue3';
import { CalendarRange, LoaderCircle, Save } from 'lucide-vue-next';
...
// Dias letivos são únicos por ano (toda a rede).
const registroAtual = computed(() => props.diasLetivos.find((d) => d.dlt_anl_id === selectedAnlId.value) ?? null);
...
watch([selectedAnlId, periodos], carregar, { immediate: true });
    ...
    <div class="grid gap-4 rounded-xl border bg-card p-6 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-semibold">Dias Letivos</h3>
            </div>
        ...
        <p class="text-xs text-muted-foreground">
            Quantidade de dias letivos do ano por mês e por período — <strong>única para toda a rede</strong>. Os totais são somados automaticamente.
        </p>
        ...
        <div class="flex justify-end">
            <Button type="button" size="sm" class="bg-indigo-600 hover:bg-indigo-700" :disabled="processing || !selectedAnlId" @click="salvar">
