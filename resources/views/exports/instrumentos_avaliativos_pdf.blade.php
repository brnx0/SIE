...
<body>
<div class="header">
    <h1>Instrumentos Avaliativos</h1>
...
<table>
    ...
    <tbody>
        @forelse($instrumentos as $i)
        <tr>
            <td><strong>{{ $i->iav_nome }}</strong></td>
            <td style="text-align:center">
                <span class="badge {{ $i->iav_fl_ativo ? 'badge-active' : 'badge-inactive' }}">
                    {{ $i->iav_fl_ativo ? 'Ativo' : 'Inativo' }}
                </span>
            </td>
        </tr>
        @empty
        <tr><td colspan="2" style="text-align:center;padding:20px;color:#94a3b8">Nenhum registro encontrado.</td></tr>
        @endforelse
    </tbody>
</table>
<div class="footer">
