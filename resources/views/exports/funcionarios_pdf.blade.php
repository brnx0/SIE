...
<table>
    ...
    <tbody>
        ...
        @php $cpf = $f->fun_cpf ? preg_replace('/(\d{3})(\d{3})(\d{3})(\d{2})/', '$1.$2.$3-$4', $f->fun_cpf) : '—'; @endphp
        <tr>
            <td><strong>{{ $f->fun_nome }}</strong></td>
            <td>{{ $cpf }}</td>
            <td>{{ $f->municipioNascimento?->mun_nome ?? '—' }}</td>
            <td>{{ $f->municipioNascimento?->mun_uf ?? '—' }}</td>
            <td style="text-align:center">
                <span class="badge {{ $f->fun_fl_ativo ? 'badge-active' : 'badge-inactive' }}">
