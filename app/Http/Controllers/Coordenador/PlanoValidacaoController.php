...
class PlanoValidacaoController extends Controller
{
    ...
    {
        $user = request()->user();
        $rolesPermitidos = ['coordenador', 'admin'];
        abort_unless($user && in_array($user->role, $rolesPermitidos, true) && $user->fun_id, 403, 'Acesso restrito a coordenadores pedagógicos.');
    }
