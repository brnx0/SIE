...
class FormacaoClassesAtividadeController extends Controller
{
    public function form(): Response
    {
        $user = auth()->user();

        return Inertia::render('relatorios/FormacaoClassesAtividade/Form', [
    ...
    public function gerar(Request $request): Response
    {
        ...
        $turmas = $turmas
            ->sortBy(fn ($t) => [$t->tur_turno, $t->tur_nome])
            ->values();
        ...
        $linhas = $turmas->map(function (Turma $t, $i) use ($totalPorAluno, $dtCorte) {
            ...
            $atividades = $t->atividades
                ->map(fn ($ta) => $ta->atividade?->atv_descricao)
                ->filter()
                ->implode(', ') ?: '—';
        ...
        return Inertia::render('relatorios/FormacaoClassesAtividade/Resultado', [
